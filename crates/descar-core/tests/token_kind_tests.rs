...
use descar_core::tokens::parsers::numeric::is_valid_integer_literal;
use descar_core::tokens::parsers::suffix::{handle_suffix, split_numeric_and_suffix};
...
fn test_unknown_suffix_for_handle_suffix() {
    ...
    assert!(handle_suffix("123", Some("x")).is_err());
}

#[test]
fn test_malformed_numeric_part_handle_unsigned_suffix() {
    // Fixed typo in test name
    assert!(handle_suffix("123.45", Some("u")).is_err());
}
...
fn test_number_with_invalid_suffix() {
    ...
    assert_eq!(lex.next().unwrap(), Err(LexError::InvalidNumberSuffix));
}
