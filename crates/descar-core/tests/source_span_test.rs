...
fn contains_uses_half_open_interval() {
    let span = Span::new(location(1, 1, 10), location(1, 6, 15));
...
fn overlap_handles_partial_containment_and_touching_boundaries() {
    let base = Span::new(location(1, 1, 10), location(1, 1, 20));
    let left = Span::new(location(1, 1, 5), location(1, 1, 12));
    let contained = Span::new(location(1, 1, 12), location(1, 1, 18));
    let right = Span::new(location(1, 1, 18), location(1, 1, 25));
    let touching_left = Span::new(location(1, 1, 0), location(1, 1, 10));
    let touching_right = Span::new(location(1, 1, 20), location(1, 1, 30));
...
fn merge_returns_minimum_covering_span() {
    let first = Span::new(location(2, 1, 10), location(2, 5, 20));
    let second = Span::new(location(1, 8, 5), location(3, 2, 25));
    assert_eq!(first.merge(&second).start(), second.start());
    assert_eq!(first.merge(&second).end(), second.end());
    let nested = Span::new(location(2, 2, 12), location(2, 3, 18));
...
fn extracts_ascii_and_empty_ranges() {
    ...
    let span = Span::new(location(1, 1, 0), location(1, 1, 5));
