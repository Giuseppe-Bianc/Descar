# Specifica tecnica Fase 03, Raccolta delle dichiarazioni e costruzione iniziale della symbol table

Serie: Descar Type Checker Technical Specifications

Document ID: DTC-03

Phase slug: declaration_collection
...
Registra tutte le dichiarazioni note negli scope già costruiti, prima della risoluzione degli usi.

- Creare i Symbol record.

- Inserire i simboli nello scope owner.

- Gestire overload sets quando ammessi.

- Registrare firme sintattiche e riferimenti ai nodi dichiarativi.

- Garantire la disponibilità di forward e recursive references.

## 2. Responsabilità specifiche

Responsibility 1: Creare i Symbol record.

Responsibility 2: Inserire i simboli nello scope owner.

Responsibility 3: Gestire overload sets quando ammessi.

Responsibility 4: Registrare firme sintattiche e riferimenti ai nodi dichiarativi.

Responsibility 5: Garantire la disponibilità di forward e recursive references.

## 3. Perimetro funzionale

### 3.1 Incluso

- `VarDeclaration`, `Function`, `MainFunction`, `Parameter` e dichiarazioni aggregate future.

- Duplicate declaration detection.

- Symbol flags e declaration identity.

### 3.2 Escluso

- Risoluzione dei type references contenuti nelle dichiarazioni.

- Validazione completa delle firme.

- Risoluzione degli usi nei body.
...
- Declaration -> SymbolId mapping.

- Overload sets.

- Declaration inventory globale.
...
for declaration in declarationsPreorder(ast) { scope=currentScope(declaration); symbol=createSymbolSkeleton(declaration); validateNamespaceCollision(scope,symbol); insert(scope,symbol); mapDecl(declaration,symbol); }
...
- Function declaration after its call site.

- Multiple functions with same name and distinct signatures.
...
f() calls g(); g() calls f();
Both symbols must exist before either body is type-checked.
...
ACCEPT: Forward reference.

ACCEPT: Self recursion.

ACCEPT: Mutual recursion.

ACCEPT: Duplicate locals.

ACCEPT: Overload set creation.

ACCEPT: Correct binding of declaration node to SymbolId.
...
- Regole di overload equivalence.
...
| SymbolArena. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Naming and declaration rules. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
...
| --- | --- | --- | --- |
| DECL-REQ-001 | MUST | ogni dichiarazione semantica deve avere un SymbolId stabile. | Implementable and testable. |
| DECL-REQ-002 | MUST | la lookup structure deve supportare cardinalità > 1 quando esistono overload. | Implementable and testable. |
| DECL-REQ-003 | MUST | recursive declarations devono essere presenti prima del body check. | Implementable and testable. |
| DECL-REQ-004 | MUST | il simbolo non può essere usato come già tipizzato se il relativo TypeId è ancora unresolved. | Implementable and testable. |
