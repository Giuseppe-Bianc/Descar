# Specifica tecnica Fase 11, Risoluzione degli overload

Serie: Descar Type Checker Technical Specifications

Document ID: DTC-11

Phase slug: overload_resolution
...
Seleziona una singola funzione, metodo o costruttore tra le candidate applicabili in modo deterministico e spiegabile.

- Enumerare candidate set.

- Verificare applicabilità.

- Integrare generic inference.

- Calcolare conversion costs.

- Rankare le candidate.

- Rilevare no-match e ambiguity.

## 2. Responsabilità specifiche

Responsibility 1: Enumerare candidate set.

Responsibility 2: Verificare applicabilità.

Responsibility 3: Integrare generic inference.

Responsibility 4: Calcolare conversion costs.

Responsibility 5: Rankare le candidate.

Responsibility 6: Rilevare no-match e ambiguity.

## 3. Perimetro funzionale

### 3.1 Incluso

- Function and method call candidates.

- Constructor candidates future.

- Overloaded operators future.

### 3.2 Escluso

- Name lookup iniziale.

- Body checking della function selezionata.
...
- Inferred type arguments if solved by the generic phase.

- Ranking trace for diagnostics.
...
candidates=enumerate(); viable=filter(isApplicable); scored=score(viable); best=best(scored); if zero no-match; if >1 equally-best ambiguous; else select.
...
- One exact, one widening.

- Generic and non-generic candidate.

- Expected return type disambiguation if the language allows it.

- Recursive call within function declaration.
...
sort<T: Comparable>(T)
T = opaque type without Comparable
=> bound failure
...
ACCEPT: Overload matrix.

ACCEPT: Tie ambiguity.

ACCEPT: Best conversion selection.

ACCEPT: Generic versus non-generic candidate.

ACCEPT: Method receiver ranking.
...
- Exact overload ranking rules.

- Whether return type participates.

- Whether generic candidates outrank non-generic ones.
...
| --- | --- | --- | --- |
| Resolved callee symbol/overload set. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Argument expressions and types. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Expected return context optional. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Conversion subsystem. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Generic constraint solver. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
...
| --- | --- | --- | --- |
| OVL-REQ-001 | MUST | only viable candidates participate in ranking. | Implementable and testable. |
| OVL-REQ-002 | MUST | ranking must be deterministic. | Implementable and testable. |
| OVL-REQ-003 | MUST | ambiguity must be reported when no unique best candidate exists. | Implementable and testable. |
| OVL-REQ-004 | MUST | return type alone must not break ambiguity unless explicitly allowed by the language. | Implementable and testable. |
| OVL-REQ-005 | MUST | conversion quality must be compared consistently across all arguments. | Implementable and testable. |
