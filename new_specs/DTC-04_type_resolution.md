# Specifica tecnica Fase 04, Risoluzione dei riferimenti di tipo

Serie: Descar Type Checker Technical Specifications

Document ID: DTC-04

Phase slug: type_resolution
...
Trasforma la sintassi dei tipi in TypeId canonici o strutture interne parametriche.

- Risoluzione dei tipi primitivi e custom.

- Risoluzione di Array, Vector e compositi futuri.

- Canonicalizzazione e interning.

- Verifica del numero e validità dei type arguments quando il linguaggio li introdurrà.

## 2. Responsabilità specifiche

Responsibility 1: Risoluzione dei tipi primitivi e custom.

Responsibility 2: Risoluzione di Array, Vector e compositi futuri.

Responsibility 3: Canonicalizzazione e interning.

Responsibility 4: Verifica del numero e validità dei type arguments quando il linguaggio li introdurrà.
...
- Array element type e size expression come dipendenza semantica.

- Alias resolution se introdotti.
...
- Resolved TypeId graph.

- Type reference -> TypeId mapping.

- ErrorType/UnresolvedType per recovery.
...
resolve(typeSyntax,env) { match typeSyntax { Primitive=>builtin; Custom(name)=>lookupType(name); Array(e,s)=>Array(resolve(e),resolveArraySize(s)); Vector(e)=>Vector(resolve(e)); Void=>Void; NullPtr=>NullPtr; } }
...
- Type name resolves to non-type symbol.
...
- Self-referential custom type.

- Mutually recursive types.

- Zero-sized or negative array dimensions.

- Custom name shadowed by value namespace.
...
foo(i32): bool
foo(i32): string
=> invalid when return-only overloading is forbidden
...
ACCEPT: Primitive canonicalization.

ACCEPT: Nested arrays/vectors.

ACCEPT: Unknown custom types.

ACCEPT: Type namespace collisions.
...
- Nominal versus structural custom types.

- Generic type syntax not present in current AST.
...
| --- | --- | --- | --- |
| Type syntax. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Scope environment. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Symbol table. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Type arena/interner. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
...
| --- | --- | --- | --- |
| P04-REQ-001 | MUST | TYPE-REF-REQ-001: ogni type reference risolto deve puntare a un TypeId valido. | Implementable and testable. |
| P04-REQ-002 | MUST | TYPE-REF-REQ-002: la canonical equality non deve dipendere dalla spelling originale. | Implementable and testable. |
| P04-REQ-003 | MUST | TYPE-REF-REQ-003: recursive type aliases devono essere rilevati tramite visited stack. | Implementable and testable. |
| P04-REQ-004 | MUST | TYPE-REF-REQ-004: array size deve essere verificata secondo una regola separata se richiede constant evaluation. | Implementable and testable. |
