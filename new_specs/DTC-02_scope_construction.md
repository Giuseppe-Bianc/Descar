# Specifica tecnica Fase 02, Costruzione degli scope

Serie: Descar Type Checker Technical Specifications

Document ID: DTC-02

Phase slug: scope_construction
...
Costruisce la struttura gerarchica degli scope lessicali senza ancora risolvere tutti i riferimenti ai nomi.

- Creare uno scope per ogni costrutto che introduce una regione di visibilità.

- Collegare ogni scope al parent corretto.

- Associare a ciascuno scope il nodo AST owner.

- Determinare l'ordinamento strutturale necessario a lookup e diagnostics.

- Separare i namespace quando la language design lo richiede.

## 2. Responsabilità specifiche

Responsibility 1: Creare uno scope per ogni costrutto che introduce una regione di visibilità.

Responsibility 2: Collegare ogni scope al parent corretto.

Responsibility 3: Associare a ciascuno scope il nodo AST owner.

Responsibility 4: Determinare l'ordinamento strutturale necessario a lookup e diagnostics.

Responsibility 5: Separare i namespace quando la language design lo richiede.

## 3. Perimetro funzionale

### 3.1 Incluso

- Global, function, block e altri scope eventualmente previsti dal linguaggio.

- Scope boundary sui nodi `Function`, `MainFunction`, `Block`, `For` e dichiarazioni future con scope proprio.

- Registrazione del parent scope.

### 3.2 Escluso

- Inserimento dei simboli delle dichiarazioni.

- Risoluzione degli identifier.

- Type checking dei body.
...
- Mapping AST node -> ScopeId per i nodi che introducono o possiedono scope.

- Current-scope metadata pronto per declaration collection.
...
walk(node, currentScope) { if introducesScope(node) { child=scope.new(parent=currentScope, owner=node); map[node]=child; currentScope=child; } for child in astChildren(node) walk(child,currentScope); }
...
- Scope owner duplicato.

- Parent scope assente quando richiesto.

- Struttura AST incompatibile con la policy di scoping.
...
- Blocchi annidati profondamente.

- Loop senza body dichiarativo aggiuntivo.

- Function body rappresentato come Block.

- Scope di `for` se initializer e body condividono o non condividono scope, a seconda della policy.
...
scope(child).parent == scope(owner_parent)
...
ACCEPT: Parent chain corretto per ogni scope.

ACCEPT: Shadowing strutturale non trattato ancora come errore di symbol table.

ACCEPT: Scope ID assegnato a ogni function body.

ACCEPT: Determinismo dell'albero di scope.
...
- Se `For.initializer` introduce scope proprio.

- Se funzioni dichiarate dentro blocchi sono item-like o block-local.

- Namespace separati per tipo, valore e label.
...
| --- | --- | --- | --- |
| AST. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| `SemanticContext` inizializzato. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Regole di scoping del linguaggio. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
...
| --- | --- | --- | --- |
| SCOPE-REQ-001 | MUST | ogni nodo owner di scope deve produrre esattamente uno ScopeId per compilation session. | Implementable and testable. |
| SCOPE-REQ-002 | MUST | ogni scope non-root deve avere un parent. | Implementable and testable. |
| SCOPE-REQ-003 | MUST | il parent deve essere semanticamente esterno al child. | Implementable and testable. |
| SCOPE-REQ-004 | MUST | la costruzione degli scope non deve dipendere dal risultato del type checker. | Implementable and testable. |
