# Specifica tecnica Fase 08, Type Checking delle espressioni

Serie: Descar Type Checker Technical Specifications

Document ID: DTC-08

Phase slug: expression_typing
...
Determina e verifica i tipi delle espressioni, propagando tipi dal basso verso l'alto e, quando utile, un expected type dall'alto verso il basso.

- Implementare synthesis e checking bidirezionale.

- Determinare il tipo di ogni `Expr`.

- Applicare type rules per literal, variable, unary, binary, grouping, array literal, assignment, call e indexing.

- Usare l'environment e le relazioni di type equivalence/compatibility/subtyping.

## 2. Responsabilità specifiche

Responsibility 1: Implementare synthesis e checking bidirezionale.

Responsibility 2: Determinare il tipo di ogni `Expr`.

Responsibility 3: Applicare type rules per literal, variable, unary, binary, grouping, array literal, assignment, call e indexing.

Responsibility 4: Usare l'environment e le relazioni di type equivalence/compatibility/subtyping.

## 3. Perimetro funzionale

### 3.1 Incluso

- Tutti gli `Expr` presenti nell'AST corrente.

- Future member access, constructors, casts e lambdas come extension hooks.

### 3.2 Escluso

- Costruzione degli scope.

- Name resolution di primo livello.

- Definite assignment globale al CFG level.

- Diagnostic presentation finale.
...
- ResolvedType per expression.

- Value category.

- Selected callable reference for resolved calls.

- Inserted conversion requests or semantic wrapper nodes.

- Constant value metadata where supported.
...
synthesize(expr) -> TypeInfo; check(expr,expected) -> TypeInfo; `Grouping` delegates to inner; `Literal` derives literal type; `Variable` reads symbol type; `Binary` checks operands then resolves operator; `Assign` checks target then RHS against target; `Call` invokes call resolver; `ArrayAccess` checks index and element access rule.
...
- Wrong array/vector access.

- Missing or ambiguous callable resolution.
...
- Numeric literal contextual type.

- `NullPtr` assignment to non-pointer type.

- Nested assignment.

- ErrorType propagation.

- Short-circuit boolean operators once flow-sensitive semantics are present.
...
let a: vector<i32> = [1,2,3];
array literal receives expected element type i32
...
ACCEPT: Golden type tests for every AST expression variant.

ACCEPT: Positive and negative binary operator matrix.

ACCEPT: Array literal homogeneity.

ACCEPT: Assignment compatibility.

ACCEPT: Call arity and result type.

ACCEPT: Array indexing.
...
- Exact numeric promotion matrix.

- Whether assignment is an expression and what type it yields.

- Operator overloading.

- Nullability semantics around current `NullPtr`.
...
| --- | --- | --- | --- |
| Resolved symbols. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Resolved type references. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| ExpectedType optional. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Expression AST. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Type context. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
...
| --- | --- | --- | --- |
| EXPR-REQ-001 | MUST | ogni expression visitata deve produrre `ResolvedType` o `ErrorType`. | Implementable and testable. |
| EXPR-REQ-002 | MUST | assignment richiede target assignable e RHS compatible con target type. | Implementable and testable. |
| EXPR-REQ-003 | MUST | binary operator selection deve essere centralizzata. | Implementable and testable. |
| EXPR-REQ-004 | MUST | array literal deve determinare un element type coerente e applicare expected type quando definito. | Implementable and testable. |
| EXPR-REQ-005 | MUST | call deve delegare a overload/generic subsystem quando il callee non è univoco. | Implementable and testable. |
| EXPR-REQ-006 | MUST | unknown name non deve causare panic; deve produrre ErrorType/Unresolved symbol. | Implementable and testable. |
