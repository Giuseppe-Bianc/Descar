# Specifica tecnica Fase 12, Type Checking degli statement

Serie: Descar Type Checker Technical Specifications

Document ID: DTC-12

Phase slug: statement_checking
...
Verifica gli statement e aggiorna environment locale, expected return context e flow inputs senza sostituirsi al CFG analyzer.

- Controllare `VarDeclaration`, `Function`, `If`, `While`, `For`, `Block`, `Return`, `Break`, `Continue`, `MainFunction`.

- Creare e chiudere i block/function contexts secondo gli scope pre-costruiti.

- Chiamare expression checker con gli expected types opportuni.

- Aggiornare le informazioni necessarie alla successiva flow analysis.

## 2. Responsabilità specifiche

Responsibility 1: Controllare `VarDeclaration`, `Function`, `If`, `While`, `For`, `Block`, `Return`, `Break`, `Continue`, `MainFunction`.

Responsibility 2: Creare e chiudere i block/function contexts secondo gli scope pre-costruiti.

Responsibility 3: Chiamare expression checker con gli expected types opportuni.

Responsibility 4: Aggiornare le informazioni necessarie alla successiva flow analysis.

## 3. Perimetro funzionale

### 3.1 Incluso

- Statement-local type rules.
...
- Full reachability fixpoint.

- Definite assignment global proof.
...
- Statement semantic metadata.

- Return requirements.

- Loop/control target metadata.

- Errors.

- Partially updated FlowState.
...
checkStmt(stmt,state,context) -> stateOut; dispatch by Stmt variant; call expression checker where needed; leave global path merging to FlowAnalyzer.
...
- Break/continue outside valid target.
...
- Nested `return` in loop.

- `break` from nested if inside loop.

- Variable declaration with no initializer.

- Multi-binding declaration with distinct initializers.
...
function f(): i32 { return "s"; }
=> return type mismatch

### Condition

if (1) { ... }
=> invalid when only Bool is a valid condition type
...
ACCEPT: Statement type matrix.

ACCEPT: Nested control statements.

ACCEPT: Return in all contexts.

ACCEPT: Break/continue target validation.

ACCEPT: Variable initializer checking.
...
- Whether declaration without initializer is legal.

- Whether `for` condition can be absent and what truth semantics follow.

- Whether blocks have independent scopes in every context.
...
| --- | --- | --- | --- |
| AST statement nodes. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Scope mapping. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Expression type checker. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Function signatures. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Flow state seed. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
...
| --- | --- | --- | --- |
| STMT-REQ-001 | MUST | condition of `if`, `while` and non-empty `for` must satisfy the language boolean/predicate rule. | Implementable and testable. |
| STMT-REQ-002 | MUST | `return` must be checked against enclosing function return type. | Implementable and testable. |
| STMT-REQ-003 | MUST | `break` and `continue` are valid only when a corresponding target exists. | Implementable and testable. |
| STMT-REQ-004 | MUST | block entry/exit must match the ScopeId constructed previously. | Implementable and testable. |
