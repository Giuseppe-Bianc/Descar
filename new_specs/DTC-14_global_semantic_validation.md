# Specifica tecnica Fase 14, Validazione semantica globale

Serie: Descar Type Checker Technical Specifications

Document ID: DTC-14

Phase slug: global_semantic_validation
...
Esegue controlli cross-cutting che richiedono l'intero modello semantico e non appartengono in modo esclusivo a una singola visita locale.

- Verificare coerenza globale di declarations, types, members, calls e flow metadata.

- Controllare invarianti cross-phase.

- Eseguire validation pass finali non locali.

- Rilevare collisioni o inconsistenze residue prima della finalizzazione dell'AST.

## 2. Responsabilità specifiche

Responsibility 1: Verificare coerenza globale di declarations, types, members, calls e flow metadata.

Responsibility 2: Controllare invarianti cross-phase.

Responsibility 3: Eseguire validation pass finali non locali.

Responsibility 4: Rilevare collisioni o inconsistenze residue prima della finalizzazione dell'AST.
...
- Module/program-level constraints.

- No dangling references.

- No unresolved required semantic nodes.

### 3.2 Escluso

- Nuova name resolution salvo recovery mirata.
...
- Validated SemanticModel.

- Global consistency status.
...
validate graph connectivity -> validate declaration invariants -> validate call bindings -> validate type graph -> validate flow summaries -> assert final semantic preconditions.
...
- Selected overload missing.

- Flow metadata inconsistent.
...
- Partial AST after recovery.

- Incremental or cached contexts.
...
CallExpr.selectedSymbol = #42
Symbol#42 exists and signature matches argument metadata
=> consistent

### Dangling reference

CallExpr.selectedSymbol = #42
Symbol#42 absent from arena
=> internal semantic error
...
ACCEPT: Corrupt internal model fixtures.

ACCEPT: Residual unresolved reference tests.

ACCEPT: Cross-phase consistency assertions.
...
- Which global rules belong here versus a dedicated module validation phase.

- Module/import/package semantics.

- Constant evaluation and compile-time execution integration.
...
| --- | --- | --- | --- |
| Tutti i semantic artifacts precedenti. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Diagnostics. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Symbol/Type/Scope graphs. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Flow results. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
...
| --- | --- | --- | --- |
| GLOBAL-REQ-001 | MUST | ogni SemanticRef deve puntare a un oggetto esistente o a un recovery object dichiarato. | Implementable and testable. |
| GLOBAL-REQ-002 | MUST | tutte le dichiarazioni richieste devono avere firme valide. | Implementable and testable. |
| GLOBAL-REQ-003 | MUST | nessun overload set unresolved può raggiungere lowering. | Implementable and testable. |
| GLOBAL-REQ-004 | MUST | errori già mascherati da ErrorType non devono generare nuove violazioni spurious. | Implementable and testable. |
