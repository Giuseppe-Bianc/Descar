# Specifica tecnica Fase 06, Risoluzione di membri, aggregate e relazioni di tipo

Serie: Descar Type Checker Technical Specifications

Document ID: DTC-06

Phase slug: aggregate_member_inheritance_resolution
...
Completa il modello semantico di classi, struct, enum, interfacce e membri futuri, mantenendo separata la verifica dei body.

- Costruire member tables.

- Risolvere base types e interfaces.

- Validare inheritance graph.

- Preparare member lookup e override metadata.

- Preparare constructor sets e method groups futuri.

## 2. Responsabilità specifiche

Responsibility 1: Costruire member tables.

Responsibility 2: Risolvere base types e interfaces.

Responsibility 3: Validare inheritance graph.

Responsibility 4: Preparare member lookup e override metadata.

Responsibility 5: Preparare constructor sets e method groups futuri.

## 3. Perimetro funzionale

### 3.1 Incluso

- Class/struct/enum/interface metadata.
...
- Body expression type checking.

- Runtime dispatch codegen.
...
resolveAggregates() { buildDeclaredMembers(); resolveBases(); validateGraphAcyclicity(); buildEffectiveMembers(); bindOverrides(); }
...
- Generic base type once generics are supported.

- Private member shadowing.

- Constructor absence or ambiguity.
...
ACCEPT: Simple inheritance.

ACCEPT: Cycle detection.

ACCEPT: Override success/failure.

ACCEPT: Member shadowing.

ACCEPT: Ambiguous multiple inheritance.
...
- Whether classes/inheritance will actually be added to Descar.

- Nominal versus structural member semantics.

- Single versus multiple inheritance.

- Trait/interface rules.
...
| --- | --- | --- | --- |
| Declaration inventory. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Resolved TypeIds. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
...
| --- | --- | --- | --- |
| AGG-REQ-001 | MUST | inheritance graph must be acyclic unless the language explicitly defines another relation. | Implementable and testable. |
| AGG-REQ-002 | MUST | member identity must be deterministic. | Implementable and testable. |
| AGG-REQ-003 | MUST | override compatibility must use the centralized signature compatibility service. | Implementable and testable. |
| AGG-REQ-004 | MUST | member lookup ambiguity must be representable, not silently guessed. | Implementable and testable. |
