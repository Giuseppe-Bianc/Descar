# Specifica tecnica Fase 10, Inferenza dei generics e constraint solving

Serie: Descar Type Checker Technical Specifications

Document ID: DTC-10

Phase slug: generic_inference_constraint_solving
...
Genera, raccoglie e risolve vincoli sulle variabili di tipo introdotte da generic declarations e call sites.

- Creare TypeVariables.

- Generare equality/subtype/convertible/callable constraints.

- Risolvere type arguments.

- Verificare bounds e trait/interface constraints.

- Restituire una soluzione stabile o una failure explainable.

## 2. Responsabilità specifiche

Responsibility 1: Creare TypeVariables.

Responsibility 2: Generare equality/subtype/convertible/callable constraints.

Responsibility 3: Risolvere type arguments.

Responsibility 4: Verificare bounds e trait/interface constraints.

Responsibility 5: Restituire una soluzione stabile o una failure explainable.

## 3. Perimetro funzionale

### 3.1 Incluso

- Generic functions and types future.
...
- Final overload ranking except for generic candidate viability.

- IR monomorphization/code generation.
...
- Type argument substitution map.

- Solved constraints.

- Unsolved or conflicting constraint diagnostics.
...
collect constraints -> normalize -> solve equalities -> propagate subtype constraints -> check bounds -> instantiate signature -> recheck contextual requirements.
...
- Occurs check or cyclic type variable.

- Ambiguous type solution.
...
- No constraints on a type parameter.

- Multiple incomparable solutions.

- Recursive generic types.

- Generic inference under overload candidates.
...
f(f64)
x:i32
f(x) => first candidate exact

### Ambiguity

f(i64)
f(u64)
x:literal with no committed signedness
=> ambiguous when neither candidate ranks better
...
ACCEPT: Identity generic.

ACCEPT: Two-parameter mapping.

ACCEPT: Subtype inference.

ACCEPT: Bound failure.

ACCEPT: No-information inference failure.

ACCEPT: Ambiguous inference.
...
- Whether generics are required in first implementation or staged later.

- Exact variance model.

- Constraint vocabulary.

- Solver strategy: unification, bounded search, general constraint graph.
...
| --- | --- | --- | --- |
| Generic signature. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Argument types. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Expected type. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Type relation services. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Candidate-specific constraints. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
...
| --- | --- | --- | --- |
| GEN-REQ-001 | MUST | ogni TypeVariable deve avere owner e identity univoci. | Implementable and testable. |
| GEN-REQ-002 | MUST | constraints devono conservare source origin per diagnostics. | Implementable and testable. |
| GEN-REQ-003 | MUST | solver termination deve essere garantita per un input finito secondo i limiti definiti. | Implementable and testable. |
| GEN-REQ-004 | MUST | una soluzione deve soddisfare tutti i vincoli obbligatori. | Implementable and testable. |
