# Specifica tecnica Fase 05, Validazione delle dichiarazioni e delle firme

Serie: Descar Type Checker Technical Specifications

Document ID: DTC-05

Phase slug: declaration_validation
...
Convalida che i simboli raccolti rappresentino dichiarazioni semanticamente ammissibili e producano firme interne utilizzabili.

- Validare cardinalità e forma delle dichiarazioni.

- Risolvere o validare gli elementi locali delle firme che sono già risolvibili.

- Controllare parametri, return type, mutabilità e proprietà dichiarative.

- Preparare signature objects immutabili o revisionabili secondo il modello scelto.

## 2. Responsabilità specifiche

Responsibility 1: Validare cardinalità e forma delle dichiarazioni.

Responsibility 2: Risolvere o validare gli elementi locali delle firme che sono già risolvibili.

Responsibility 3: Controllare parametri, return type, mutabilità e proprietà dichiarative.

Responsibility 4: Preparare signature objects immutabili o revisionabili secondo il modello scelto.
...
- Main function signature constraints.

- Parameter uniqueness.

- Declaration-level legality.

- Signature consistency across overloads.

### 3.2 Escluso

- Body expression typing.

- Call-site overload resolution.

- Control-flow of the function body.

- Generic inference di call sites.
...
- List of declarations eligible for body checking.
...
for symbol in declarationInventory { if functionLike(symbol) { validateParameters(); validateReturnType(); signature=buildSignature(); registerSignature(symbol,signature); } validateDeclarationAttributes(); }
...
- Non-void function without body if body required.

- Overloads differing only in return type when prohibited.
...
let x: Missing = 1;
=> unknown type, X as recovery symbol/type
...
ACCEPT: Signature snapshot tests.

ACCEPT: Duplicate parameter tests.

ACCEPT: Overload collision tests.

ACCEPT: Main signature acceptance/rejection.
...
- Overload equivalence rules.

- Method versus function namespace.

- Default parameters, variadic parameters and attributes.
...
| --- | --- | --- | --- |
| Declaration symbols. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| AST declaration nodes. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Resolved or resolvable type references. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
...
| --- | --- | --- | --- |
| SIG-REQ-001 | MUST | una signature deve avere arità, parameter types e return type coerenti. | Implementable and testable. |
| SIG-REQ-002 | MUST | due overload sono distinte solo se il linguaggio lo consente secondo la signature equivalence policy. | Implementable and testable. |
| SIG-REQ-003 | MUST | `main` deve rispettare la signature language-defined. | Implementable and testable. |
| SIG-REQ-004 | MUST | un parametro deve avere nome unico nello scope della funzione. | Implementable and testable. |
