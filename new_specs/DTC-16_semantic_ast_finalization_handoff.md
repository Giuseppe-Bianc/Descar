# Specifica tecnica Fase 16, Finalizzazione dell'AST semantico e handoff

Serie: Descar Type Checker Technical Specifications

Document ID: DTC-16

Phase slug: semantic_ast_finalization_handoff
...
Congela e rende consumabile il semantic model dalle fasi successive, eliminando ambiguità residue e rendendo esplicite conversioni e binding.

- Annotare expression con ResolvedType.

- Memorizzare SymbolId e selected callable.

- Esplicitare conversioni implicite dove previsto.

- Consolidare generic substitutions e flow metadata.

- Esportare un `SemanticResult` pronto per lowering/IR.

## 2. Responsabilità specifiche

Responsibility 1: Annotare expression con ResolvedType.

Responsibility 2: Memorizzare SymbolId e selected callable.

Responsibility 3: Esplicitare conversioni implicite dove previsto.

Responsibility 4: Consolidare generic substitutions e flow metadata.

Responsibility 5: Esportare un `SemanticResult` pronto per lowering/IR.
...
- Symbol/type/flow tables.

- Handoff contract for IR generator.
...
assertPreconditions(); materializeSemanticAnnotations(); freezeSymbolBindings(); materializeConversions(); attachFlowFacts(); buildSemanticResult(); validateHandoffContract(); return result.
...
- Unmaterialized required conversion.

- Unresolved callable.

- Inconsistent type metadata.
...
- Optional emission of semantic AST separate from syntax AST.
...
Call(f, [3])
=> Call(target=Function#7, arg[0]=Convert(3, i64), type=i64)

### Handoff gate

if unresolved_required_semantics > 0: prohibit_lowering()
...
ACCEPT: AST annotation snapshots.

ACCEPT: Round-trip semantic printer tests.

ACCEPT: Lowering precondition tests.

ACCEPT: Failure gate on unresolved semantics.
...
- Whether to mutate existing AST or create a parallel Semantic AST.

- Serialization format for semantic metadata.

- Ownership/lifetime strategy for SymbolId/TypeId.

- Handoff API shape for current IR generator.
...
| --- | --- | --- | --- |
| All prior semantic artifacts. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Final diagnostics. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
| Validated AST and semantic graph. | AST / semantic service / context | Dato consumato dalla fase | Validato prima dell'uso |
...
| --- | --- | --- | --- |
| FINAL-REQ-001 | MUST | ogni expression semanticamente valida deve avere ResolvedType. | Implementable and testable. |
| FINAL-REQ-002 | MUST | ogni resolved identifier deve avere SymbolId. | Implementable and testable. |
| FINAL-REQ-003 | MUST | ogni resolved call deve avere target identity e argument conversion metadata. | Implementable and testable. |
| FINAL-REQ-004 | MUST | lowering non deve dover ripetere overload resolution o lexical lookup. | Implementable and testable. |
| FINAL-REQ-005 | MUST | se esistono errori fatali semanticamente impeditivi, il result deve indicare che lowering è vietato. | Implementable and testable. |
